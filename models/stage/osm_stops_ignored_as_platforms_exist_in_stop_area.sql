...
SELECT
  s.osm_id
FROM stage.osm_stop_area_members AS mp
JOIN stage.osm_stop_area_members AS ms
  USING (osm_id)
JOIN stage.osm_stop_candidates_with_mode_and_type AS p
  ON mp.member_id = p.osm_id
JOIN stage.osm_stop_candidates_with_mode_and_type AS s
  ON ms.member_id = s.osm_id
WHERE
  p.mode = 'bus' AND p.type = 'platform' AND s.mode = 'bus' AND s.type = 'stop'