...
SELECT
  tram_stop.osm_id
FROM stage.osm_stop_candidates_with_mode_and_type AS tram_stop
JOIN stage.osm_stop_candidates_with_mode_and_type AS s
  ON tram_stop.name = s.name
  AND s.lat BETWEEN tram_stop.lat - 0.01 AND tram_stop.lat + 0.01
  AND s.lon BETWEEN tram_stop.lon - 0.01 AND tram_stop.lon + 0.01
WHERE
  tram_stop.type = 'stop'
